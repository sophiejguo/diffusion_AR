...
import dist
from models import DiffusionVAR, VQVAE, VectorQuantizer2
...
class DiffusionVARTrainer(object):
    def __init__(
        ...
        self.var_wo_ddp: DiffusionVAR = var_wo_ddp  # after torch.compile
    ...
    # @torch.no_grad()
    # def eval_ep_old(self, ld_val: DataLoader):
    #     tot = 0
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        for inp_B3HW, label_B in ld_val:
            ...
            L_mean += self.forward_diffusion_loss(x_BLV, gt_embed_Bl, mask) ## x, t, c 
    ...
    def forward_diffusion_loss(self, z, target, mask):
        ...
        mask = torch.zeros(bsz, seq_len) ## overwrite mask, fix this later to be correct causal masking 
    ...
    def train_step(
        ...
        ## targets are now embeddings instead of quantized indices 
        x_BLCv_wo_first_l: Ten = self.quantize_local.idxBl_to_var_input(gt_idx_Bl) ## for teacher forcing

        ## embedidng is nn.Embedding(vocab_size, Cvae)
        # get_BL is a list of in indices, I want to get the embedding of each index

        # gt_embed_Bl: List[Ten] = [self.vae_local.quantize.embedding[idx] for idx in gt_idx_Bl]
        # print(gt_embed_Bl[0].shape)

        gt_embed_Bl = self.vae_local.quantize.embedding(gt_BL).cuda()
        # print(gt_embed_BL.shape)

        with self.var_opt.amp_ctx:
            self.var_wo_ddp.forward
            x_BLV = self.var(label_B, x_BLCv_wo_first_l)
            
            mask = torch.ones_like(x_BLV).cuda() ## dummy mask for now
            
            loss = self.forward_diffusion_loss(x_BLV, gt_embed_Bl, mask) ## x, t, c 

            ## doesn't incorporate diffusion batch mul the way forward_diffusion_loss does? 
            # loss = self.diff_loss(z=x_BLV.view(-1,V), target=gt_BL.view(-1), mask=mask.view(-1)).view(B,-1)
        ...
        if it == 0 or it in metric_lg.log_iters:
            ...
            pred_BL = x_BLV.data.argmax(dim=-1)
