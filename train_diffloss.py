...
def build_everything(args: arg_util.Args):
    ...
    from trainer_diffloss import DiffusionVARTrainer
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        is_val_and_also_saving = (ep + 1) % 10 == 0 or (ep + 1) == args.ep ## log every 2
        if is_val_and_also_saving:
            val_loss_mean, tot, cost = trainer.eval_ep(ld_val)
            wandb.log({
                "val_loss_mean": val_loss_mean,
            })
            val_loss_tail = val_loss_mean ## dummy
            val_acc_mean = val_loss_mean ## dummy
            val_acc_tail = val_loss_mean ## dummy
            best_updated = best_val_loss_tail > val_loss_mean
