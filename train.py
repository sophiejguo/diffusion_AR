...
def build_everything(args: arg_util.Args):
    ...
    print("auto_resume state:" + str(args.auto_resume))
    if args.auto_resume:
    ...
    from var_models import VAR, VQVAE, build_vae_var
    from trainer import VARTrainer
    from utils.amp_sc import AmpOptimizer
    from utils.lr_controler import filter_params
    
    vae_local, var_wo_ddp = build_vae_var(
        V=args.V, Cvae=32, ch=128, share_quant_resi=4,        # hard-coded VQVAE hyperparameters
        device=dist.get_device(), patch_nums=args.patch_nums,
        num_classes=num_classes, depth=args.depth, shared_aln=args.saln, attn_l2_norm=args.anorm,
        flash_if_available=args.fuse, fused_if_available=args.fuse,
        init_adaln=args.aln, init_adaln_gamma=args.alng, init_head=args.hd, init_std=args.ini,
    )
    # path of the vae checkpoint
    vae_ckpt = './taming-transformers/logs/2025-05-12T21-53-13_multiscale_vqvae/checkpoints/last.ckpt' 
    if dist.is_local_master():
        if not os.path.exists(vae_ckpt):
            print(f'Downloading VAE checkpoint from Hugging Face...')
            os.system(f'wget https://huggingface.co/FoundationVision/var/resolve/main/{vae_ckpt}')
    dist.barrier()
    checkpoint = torch.load(vae_ckpt, map_location='cpu')
    vae_local.load_state_dict(checkpoint['state_dict'], strict=False)
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        if is_val_and_also_saving:
            ...
            if dist.is_local_master():
                local_out_ckpt = os.path.join(args.tb_log_dir_path, 'ar-ckpt-last.pth')
                local_out_ckpt_best = os.path.join(args.tb_log_dir_path, 'ar-ckpt-best.pth')
            ...
            with torch.no_grad():
                B = 8
                sample_labels = torch.tensor([0, 1, 2, 3, 4, 5, 6, 7], device=args.device)
                with torch.autocast('cuda', enabled=True, dtype=torch.float16, cache_enabled=True):
                    recon_B3HW = trainer.var_wo_ddp.autoregressive_infer_cfg(
                        B=B, 
                        label_B=sample_labels, 
                        cfg=2.5,
                        top_k=900, 
                        top_p=0.95, 
                        g_seed=ep,  # use epoch as seed
                        more_smooth=True
                    )
                
                grid = torchvision.utils.make_grid(recon_B3HW, nrow=4, padding=0, pad_value=1.0)
                grid = grid.permute(1, 2, 0).mul_(255).cpu().numpy()
                grid = grid.astype(np.uint8)
                
                # Log to wandb
                wandb.log({
                    "validation_images": wandb.Image(grid),
                    "epoch": ep
                })
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    from trainer import VARTrainer
    from utils.lr_controler import lr_wd_annealing
    trainer: VARTrainer
    ...
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        ...
        if it % 100 == 0:
            with torch.no_grad():
                ...
                B = 8  # batch size for visualization
                sample_labels = torch.tensor([0, 1, 2, 3, 4, 5, 6, 7], device=args.device)
                with torch.autocast('cuda', enabled=True, dtype=torch.float16, cache_enabled=True):
                    recon_B3HW = trainer.var_wo_ddp.autoregressive_infer_cfg(
                        B=B, 
                        ...
                        top_k=900, 
                        top_p=0.95, 
                        g_seed=it,  # use iteration as seed
                        more_smooth=True
                    )
                
                # Convert to grid and log to wandb
                grid = torchvision.utils.make_grid(recon_B3HW, nrow=4, padding=0, pad_value=1.0)
                grid = grid.permute(1, 2, 0).mul_(255).cpu().numpy()
                grid = grid.astype(np.uint8)
